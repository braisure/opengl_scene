...
public class ModelLoader {
     ...
     * 从obj文件中加载物体模型
     */
    public static CommonModel loadFromObjFile(Resources r, String fname, int program)
    {
        ...
        try {
            ...
            while ((temps = br.readLine()) != null)
            {
                ...
                if (tempsa[0].trim().equals("v")) {         // 此行为顶点坐标
                    ...
                    // 若为顶点坐标行，则提取出此顶点的XYZ坐标添加到原始顶点坐标列表中
                    ...
                    alv.add(Float.parseFloat(tempsa[3]));
		      		...
		      		// 若为三角形面行，则根据组成面的顶点的索引从原始顶点坐标列表中提取相应的
                    // 顶点坐标值添加到结果顶点坐标列表中，同时根据三个顶点的坐标计算出此面的
                    // 法向量并添加到平均前各个索引对应的点的法向量集合组成的Map中
                    ...
                    for (int tempIndex : index) {
                        ...
                        if (hsn == null) {
                            hsn = new HashSet<Normal>();  //若集合不存在则创建
                        }

                        // 将此点的法向量添加到集合中
                        // 由于Normal类重写了equals方法，因此同样的法向量不会重复出现在此点对应的法向量集合中
            ...
            // 创建模型对象
            model = new CommonModel(vXYZ, nXYZ, program);
